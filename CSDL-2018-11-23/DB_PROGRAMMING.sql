...
SELECT MONTH(HOSO_DANGKY_NGAY_DANGKY) THANG
		...
		WHERE 
			HOSO_DANGKY.LOAI_DANGKY_ID = LOAI_DANGKY.LOAI_DANGKY_ID 
			AND HOSO_DANGKY.COQUAN_ID = 1
			AND YEAR(HOSO_DANGKY_NGAY_DANGKY) = 2018
		GROUP BY 
			MONTH(HOSO_DANGKY_NGAY_DANGKY)
			,LOAI_GIAYTO_ID
...
DECLARE CURSOR_LOAI CURSOR FOR 
SELECT LOAI_GIAYTO_ID,LOAI_GIAYTO_TEN 
FROM LOAI_GIAYTO 
DECLARE @ID INT,@NAME NVARCHAR(50)
OPEN CURSOR_LOAI
FETCH NEXT FROM CURSOR_LOAI INTO @ID,@NAME
WHILE @@FETCH_STATUS = 0
BEGIN
	SELECT * FROM LOAI_GIAYTO WHERE LOAI_GIAYTO_ID = @ID
	FETCH NEXT FROM CURSOR_LOAI INTO @ID, @NAME
END
CLOSE CURSOR_LOAI
DEALLOCATE CURSOR_LOAI

DROP FUNCTION GET_HOSO_DANGKY_NAM
CREATE FUNCTION GET_HOSO_DANGKY_NAM(@NAM INT,@COQUAN INT)
RETURNS @table TABLE (LOAI_GIAYTO_ID INT, LOAI_GIAYTO_TEN NVARCHAR(50), THANG INT, SOLUONG INT)
AS
BEGIN
	DECLARE @THANG INT
	DECLARE @SOLUONG INT
	...
	BEGIN
		...
		FROM HOSO_DANGKY INNER JOIN LOAI_DANGKY ON HOSO_DANGKY.LOAI_DANGKY_ID = LOAI_DANGKY.LOAI_DANGKY_ID
		WHERE HOSO_DANGKY.COQUAN_ID = @COQUAN AND YEAR(HOSO_DANGKY_NGAY_DANGKY) = @NAM AND LOAI_DANGKY.LOAI_GIAYTO_ID = @ID AND MONTH(HOSO_DANGKY_NGAY_DANGKY) = @THANG
		INSERT @table SELECT @ID,@NAME,@THANG,@SOLUONG
		SET @THANG = @THANG + 1
		END
		FETCH NEXT FROM CURSOR_LOAI INTO @ID, @NAME
	END
	...
	RETURN
END
SELECT * FROM GET_HOSO_DANGKY_NAM(2018,1)
