...
AS
	RETURN ( SELECT *
			 ...
			 WHERE KS.HOSO_DANGKY_ID = @idHSDK
			)
...
AS
	...
	SELECT 
			...
			,@me_ID = me.CONGDAN_ID
	FROM DANGKY_KHAISINH ks LEFT JOIN CONGDAN cha ON KS.CHA_CMND = cha.CONGDAN_CMND LEFT JOIN CONGDAN me ON KS.ME_CMND = me.CONGDAN_CMND,HOSO_DANGKY hsdk
...
EXEC TimKiem_HoTich 'KS0001','KS-T12'
...
RETURNS @table table (
					LOAI_GIAYTO_ID INT						--1
					, MA NVARCHAR(10)						--2
					, SOQUYEN NVARCHAR(10)					--3
					, CONGDAN_HOTEN NVARCHAR(255)			--4
					, CONGDAN_GIOITINH BIT					--5
					, CONGDAN_NGAYSINH DATE					--6
					, CONGDAN_QUEQUAN NVARCHAR(50)			--7
					, CONGDAN_NOISINH NVARCHAR(255)			--8
					, CONGDAN_DANTOC NVARCHAR(50)			--9
					, CONGDAN_QUOCTICH NVARCHAR(50)			--10
					, CHA_HOVATEN NVARCHAR(255)				--11
					, CHA_DANTOC NVARCHAR(50)				--12
					, CHA_QUOCTICH NVARCHAR(50)				--13
					, CHA_THANHPHO NVARCHAR(50)				--14
					, CHA_QUAN NVARCHAR(50)					--15
					, CHA_PHUONG NVARCHAR(50)				--16
					, CHA_DIACHI NVARCHAR(255)				--17
					, ME_HOVATEN NVARCHAR(255)				--18
					, ME_DANTOC NVARCHAR(50)				--19
					, ME_QUOCTICH NVARCHAR(50)				--20
					, ME_THANHPHO NVARCHAR(50)				--21
					, ME_QUAN NVARCHAR(50)					--22
					, ME_PHUONG NVARCHAR(50)				--23
					, ME_DIACHI NVARCHAR(255)				--24
					, NGAY_KI DATETIME						--25
					, NGUOI_XULY_HOTEN NVARCHAR(100)		--26
					, NGUOI_KY_HOTEN NVARCHAR(100)			--27
					, CONGDAN_NOISINH_QUOCGIA NVARCHAR(50)	--28
					, CONGDAN_NOISINH_THANHPHO NVARCHAR(50)	--29
					, CONGDAN_NOISINH_QUAN NVARCHAR(50)		--30
					, CONGDAN_NOISINH_PHUONG NVARCHAR(50)	--31
					, CONGDAN_NOISINH_DIACHI NVARCHAR(255)	--32
					, ME_NGAYSINH DATE						--33
					, CHA_NGAYSINH DATE						--34
					, COQUAN_TEN NVARCHAR(255)				--35
					, NGYEUCAU_TEN NVARCHAR(255)			--36
					)
AS
	BEGIN
		INSERT @table 
			SELECT 
				GKS.LOAI_GIAYTO_ID							--1
				, GKS.GIAY_HOTICH_MA						--2
				, GKS.GIAY_HOTICH_SOQUYEN					--3
				, CON.CONGDAN_HOVATEN						--4
				--, CASE CON.CONGDAN_GIOITINH		
				--	WHEN 0 THEN 'NAM'				
				--	WHEN 1 THEN 'NỮ'
				--  END
				, CON.CONGDAN_GIOITINH						--5
				, CON.CONGDAN_NGAYSINH						--6
				, CON.CONGDAN_QUEQUAN						--7
				, GKS.GIAY_KHAISINH_NOISINH					--8
				, CON.CONGDAN_DANTOC						--9
				, CON.CONGDAN_QUOCTICH						--10
				, CHA.CONGDAN_HOVATEN						--11
				, CHA.CONGDAN_DANTOC						--12
				, CHA.CONGDAN_QUOCTICH						--13
				, CHA.CONGDAN_THANHPHO						--14
				, CHA.CONGDAN_QUAN							--15
				, CHA.CONGDAN_PHUONG						--16
				, CHA.CONGDAN_DIACHI						--17
				, ME.CONGDAN_HOVATEN						--18
				, ME.CONGDAN_DANTOC							--19
				, ME.CONGDAN_QUOCTICH						--20
				, ME.CONGDAN_THANHPHO						--21
				, ME.CONGDAN_QUAN							--22
				, ME.CONGDAN_PHUONG							--23
				, ME.CONGDAN_DIACHI							--24
				, GKS.GIAY_HOTICH_NGAY_KY					--25
				, NGUOI_XULY.NGUOIDUNG_HOVATEN				--26
				, NGUOI_KY.NGUOIDUNG_HOVATEN				--27
				, GKS.GIAY_KHAISINH_NOISINH_QUOCGIA			--28
				, GKS.GIAY_KHAISINH_NOISINH_THANHPHO		--29
				, GKS.GIAY_KHAISINH_NOISINH_QUAN			--30
				, GKS.GIAY_KHAISINH_NOISINH_PHUONG			--31
				, GKS.GIAY_KHAISINH_NOISINH_DIACHI			--32
				, ME.CONGDAN_NGAYSINH						--33
				, CHA.CONGDAN_NGAYSINH						--34
				, COQUAN_TEN								--35
				, HSKHAISINH.NGUOIYEUCAU_TEN				--36
			FROM	
				(
					SELECT *
					FROM GIAY_KHAISINH , (
				...
				INNER JOIN (
							SELECT HOSO_DANGKY.*, DANGKY_KHAISINH.NGUOIYEUCAU_TEN
							FROM HOSO_DANGKY INNER JOIN DANGKY_KHAISINH 
							ON HOSO_DANGKY.HOSO_DANGKY_ID = DANGKY_KHAISINH.HOSO_DANGKY_ID
							) AS HSKHAISINH ON GKS.HOSO_DANGKY_ID = HSKHAISINH.HOSO_DANGKY_ID
				INNER JOIN COQUAN ON GKS.COQUAN_ID = COQUAN.COQUAN_ID
				LEFT JOIN CONGDAN AS CHA ON CHA.CONGDAN_ID = GKS.CHA_ID
				LEFT JOIN CONGDAN AS ME ON ME.CONGDAN_ID = GKS.ME_ID
		...
		RETURN
	END
SELECT * FROM Get_Giay_KhaiSinh (17)
...
AS
	SELECT GIAY_HOTICH.*,NG_XULY.NGUOIDUNG_HOVATEN AS NG_XULY_TEN,NG_KY.NGUOIDUNG_HOVATEN AS NG_KY_TEN
	FROM GIAY_HOTICH 
	INNER JOIN NGUOIDUNG NG_XULY ON GIAY_HOTICH.GIAY_HOTICH_NGUOI_XULY = NG_XULY.NGUOIDUNG_ID
	INNER JOIN NGUOIDUNG NG_KY ON GIAY_HOTICH.GIAY_HOTICH_NGUOI_KY = NG_KY.NGUOIDUNG_ID
	WHERE GIAY_HOTICH.COQUAN_ID = @maCoQuan
	ORDER BY GIAY_HOTICH_NGAY_KY DESC

EXEC GET_HOTICH_COQUAN 1

--2018/11/24
--Tìm kiếm công dân dựa trên CMND
DROP FUNCTION TIMKIEM_CONGDAN
CREATE FUNCTION TIMKIEM_CONGDAN(@cmnd nvarchar(20))
RETURNS table
AS
	RETURN (
		SELECT *
		FROM CONGDAN
		WHERE CONGDAN_CMND = @cmnd AND CONGDAN_CONSONG = 1
		)
select * from TIMKIEM_CONGDAN('321564307')

--2018/12/07
--Hàm cho thống kê
DROP FUNCTION GET_HOSO_DANGKY_THANG
CREATE FUNCTION GET_HOSO_DANGKY_THANG(@coQuanID INT,@thang INT)
RETURNS TABLE
AS
RETURN (
		SELECT MONTH(HOSO_DANGKY_NGAY_DANGKY) THANG
				, LOAI_GIAYTO_ID LOAI_GIAYTO
				, COUNT(*) SOLUONG 
		FROM 
			HOSO_DANGKY
			,LOAI_DANGKY
		...
		WHERE 
			HOSO_DANGKY.LOAI_DANGKY_ID = LOAI_DANGKY.LOAI_DANGKY_ID 
			AND HOSO_DANGKY.COQUAN_ID = @coQuanID
		GROUP BY 
			MONTH(HOSO_DANGKY_NGAY_DANGKY)
			,LOAI_GIAYTO_ID
		HAVING MONTH(HOSO_DANGKY_NGAY_DANGKY) = @thang
		)

SELECT * FROM GET_HOSO_DANGKY_THANG(1,12)

DROP FUNCTION GET_THONGKE_HSDK_THANG
CREATE FUNCTION GET_THONGKE_HSDK_THANG(@coQuanId INT,@thang INT)
RETURNS TABLE
AS 
RETURN (
		SELECT THANG, LOAI_GIAYTO,SOLUONG,LOAI_GIAYTO_TEN,TONG
		FROM 
			GET_HOSO_DANGKY_THANG(@coQuanId,@thang)
			, LOAI_GIAYTO
			,(
				SELECT COUNT(*) TONG
				FROM HOSO_DANGKY
				WHERE MONTH(HOSO_DANGKY_NGAY_DANGKY) = @thang
			) AS TONG_THANG
		WHERE LOAI_GIAYTO = LOAI_GIAYTO.LOAI_GIAYTO_ID
		)
SELECT * FROM GET_THONGKE_HSDK_THANG(1,12)